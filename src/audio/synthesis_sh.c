...
u64 *synthesis_process_note(s32 noteIndex, struct NoteSubEu *noteSubEu,
                            struct NoteSynthesisState *synthesisState, s16 *aiBuf, s32 bufLen, u64 *cmd,
                            s32 updateIndex);
u64 *load_wave_samples(u64 *cmd, struct NoteSubEu *noteSubEu, struct NoteSynthesisState *synthesisState,
                       s32 nSamplesToLoad);
u64 *final_resample(u64 *cmd, struct NoteSynthesisState *synthesisState, s32 count, u16 pitch,
                    u16 dmemIn, u32 flags);
u64 *process_envelope(u64 *cmd, struct NoteSubEu *noteSubEu, struct NoteSynthesisState *synthesisState,
                      s32 nSamples, u16 inBuf, s32 headsetPanSettings, u32 flags);
u64 *note_apply_headset_pan_effects(u64 *cmd, struct NoteSubEu *noteSubEu,
                                    struct NoteSynthesisState *note, s32 bufLen, s32 flags,
                                    s32 leftRight);
...
u64 *synthesis_load_reverb_ring_buffer(u64 *cmd, u16 addr, u16 srcOffset, s32 len, s32 reverbIndex) {
    aLoadBuffer(cmd++, VIRTUAL_TO_PHYSICAL2(&gSynthesisReverbs[reverbIndex].ringBuffer.left[srcOffset]),
                addr, len);
    aLoadBuffer(cmd++,
                VIRTUAL_TO_PHYSICAL2(&gSynthesisReverbs[reverbIndex].ringBuffer.right[srcOffset]),
...
u64 *synthesis_save_reverb_ring_buffer(u64 *cmd, u16 addr, u16 destOffset, s32 len, s32 reverbIndex) {
    ...
    aSaveBuffer(cmd++, addr + DEFAULT_LEN_1CH,
                VIRTUAL_TO_PHYSICAL2(&gSynthesisReverbs[reverbIndex].ringBuffer.right[destOffset]),
                len);
    return cmd;
}
...
u64 *synthesis_execute(u64 *cmdBuf, s32 *writtenCmds, s16 *aiBuf, s32 bufLen) {
    ...
    for (i = gAudioBufferParameters.updatesPerFrame; i > 0; i--) {
        ...
        cmd = synthesis_do_one_audio_update((s16 *) aiBufPtr, chunkLen, cmd,
                                            gAudioBufferParameters.updatesPerFrame - i);
        bufLen -= chunkLen;
        aiBufPtr += chunkLen;
    }
...
u64 *synthesis_resample_and_mix_reverb(u64 *cmd, s32 bufLen, s16 reverbIndex, s16 updateIndex) {
    ...
    if (gSynthesisReverbs[reverbIndex].downsampleRate == 1) {
        cmd = synthesis_load_reverb_ring_buffer(cmd, DMEM_ADDR_WET_LEFT_CH, item->startPos,
                                                item->lengthA, reverbIndex);
        if (item->lengthB != 0) {
            cmd = synthesis_load_reverb_ring_buffer(cmd, DMEM_ADDR_WET_LEFT_CH + item->lengthA, 0,
                                                    item->lengthB, reverbIndex);
        }
        aAddMixer(cmd++, DMEM_ADDR_WET_LEFT_CH, DMEM_ADDR_LEFT_CH, DEFAULT_LEN_2CH);
        aMix(cmd++, 0x8000 + gSynthesisReverbs[reverbIndex].reverbGain, DMEM_ADDR_WET_LEFT_CH,
             DMEM_ADDR_WET_LEFT_CH, DEFAULT_LEN_2CH);
    } else {
        startPad = (item->startPos % 8u) * 2;
        paddedLengthA = ALIGN(startPad + item->lengthA, 4);

        cmd = synthesis_load_reverb_ring_buffer(
            cmd, DMEM_ADDR_RESAMPLED, (item->startPos - startPad / 2), DEFAULT_LEN_1CH, reverbIndex);
        if (item->lengthB != 0) {
            cmd = synthesis_load_reverb_ring_buffer(cmd, DMEM_ADDR_RESAMPLED + paddedLengthA, 0,
                                                    DEFAULT_LEN_1CH - paddedLengthA, reverbIndex);
        }

        aSetBuffer(cmd++, 0, DMEM_ADDR_RESAMPLED + startPad, DMEM_ADDR_WET_LEFT_CH, bufLen * 2);
        aResample(cmd++, gSynthesisReverbs[reverbIndex].resampleFlags,
                  gSynthesisReverbs[reverbIndex].resampleRate,
    ...
    if (gSynthesisReverbs[reverbIndex].panRight != 0 || gSynthesisReverbs[reverbIndex].panLeft != 0) {
        // Leak some audio from the left reverb channel into the right reverb channel and vice versa
        // (pan)
        aDMEMMove(cmd++, DMEM_ADDR_WET_LEFT_CH, DMEM_ADDR_RESAMPLED, DEFAULT_LEN_1CH);
        aMix(cmd++, gSynthesisReverbs[reverbIndex].panRight, DMEM_ADDR_WET_RIGHT_CH,
             DMEM_ADDR_WET_LEFT_CH, DEFAULT_LEN_1CH);
        aMix(cmd++, gSynthesisReverbs[reverbIndex].panLeft, DMEM_ADDR_RESAMPLED, DMEM_ADDR_WET_RIGHT_CH,
             DEFAULT_LEN_1CH);
    }
    return cmd;
}
...
u64 *synthesis_load_reverb_samples(u64 *cmd, s16 reverbIndex, s16 updateIndex) {
    ...
    cmd = synthesis_load_reverb_ring_buffer(cmd, DMEM_ADDR_RESAMPLED, item->startPos, item->lengthA,
                                            reverbIndex);
    if (item->lengthB != 0) {
        // Ring buffer wrapped
        cmd = synthesis_load_reverb_ring_buffer(cmd, DMEM_ADDR_RESAMPLED + item->lengthA, 0,
                                                item->lengthB, reverbIndex);
    }
    return cmd;
}
...
u64 *synthesis_save_reverb_samples(u64 *cmd, s16 reverbIndex, s16 updateIndex) {
    ...
    switch (gSynthesisReverbs[reverbIndex].downsampleRate) {
        case 1:
            ...
            cmd = synthesis_save_reverb_ring_buffer(cmd, DMEM_ADDR_WET_LEFT_CH, item->startPos,
                                                    item->lengthA, reverbIndex);
            if (item->lengthB != 0) {
                // Ring buffer wrapped
                cmd = synthesis_save_reverb_ring_buffer(cmd, DMEM_ADDR_WET_LEFT_CH + item->lengthA, 0,
                                                        item->lengthB, reverbIndex);
            }
            break;
...
u64 *func_sh_802EDF24(u64 *cmd, s16 reverbIndex, s16 updateIndex) {
    ...
    item = &reverb->items[reverb->curFrame][updateIndex];
    // Put the oldest samples in the ring buffer into the wet channels
    cmd = synthesis_save_reverb_ring_buffer(cmd, DMEM_ADDR_RESAMPLED, item->startPos, item->lengthA,
                                            reverbIndex);
    if (item->lengthB != 0) {
        // Ring buffer wrapped
        cmd = synthesis_save_reverb_ring_buffer(cmd, DMEM_ADDR_RESAMPLED + item->lengthA, 0,
                                                item->lengthB, reverbIndex);
    }
    return cmd;
}
...
u64 *synthesis_do_one_audio_update(s16 *aiBuf, s32 bufLen, u64 *cmd, s32 updateIndex) {
    ...
    for (j = 0; j < gNumSynthesisReverbs; j++) {
        ...
        for (; i < notePos; i++) {
            ...
            if (j == gNoteSubsEu[temp + noteIndices[i]].reverbIndex) {
                cmd = synthesis_process_note(noteIndices[i], &gNoteSubsEu[temp + noteIndices[i]],
        ...
        if (gSynthesisReverbs[j].useReverb != 0) {
            if (gSynthesisReverbs[j].unk100 != NULL) {
                ...
                aFilter(cmd++, gSynthesisReverbs[j].resampleFlags, DMEM_ADDR_WET_LEFT_CH,
                        gSynthesisReverbs[j].unk108);
            }
            if (gSynthesisReverbs[j].unk104 != NULL) {
                aFilter(cmd++, 0x02, bufLen * 2, gSynthesisReverbs[j].unk104);
                aFilter(cmd++, gSynthesisReverbs[j].resampleFlags, DMEM_ADDR_WET_RIGHT_CH,
                        gSynthesisReverbs[j].unk10C);
            }
            ...
            if (gSynthesisReverbs[j].unk5 != -1) {
                if (gSynthesisReverbs[gSynthesisReverbs[j].unk5].downsampleRate == 1) {
                    ...
                    aMix(cmd++, gSynthesisReverbs[j].unk08, DMEM_ADDR_WET_LEFT_CH, DMEM_ADDR_RESAMPLED,
                         DEFAULT_LEN_2CH);
    ...
    for (; i < notePos; i++) {
        struct NoteSubEu *noteSubEu2 =
            &gNoteSubsEu[updateIndex * gMaxSimultaneousNotes + noteIndices[i]];
        cmd = synthesis_process_note(noteIndices[i], noteSubEu2, &gNotes[noteIndices[i]].synthesisState,
...
u64 *synthesis_process_note(s32 noteIndex, struct NoteSubEu *noteSubEu,
                            struct NoteSynthesisState *synthesisState, UNUSED s16 *aiBuf, s32 bufLen,
                            u64 *cmd, s32 updateIndex) {
    UNUSED s32 pad0[3];
    ...
    if (noteSubEu->isSyntheticWave) {
        ...
        noteSamplesDmemAddrBeforeResampling =
            (synthesisState->samplePosInt * 2) + DMEM_ADDR_UNCOMPRESSED_NOTE;
        ...
        for (curPart = 0; curPart < nParts; curPart++) {
            ...
            while (nAdpcmSamplesProcessed != samplesLenAdjusted) {
                ...
                if (t0 != 0) {
                    ...
                    if (audioBookSample->medium == 0) {
                        ...
                        v0_2 =
                            dma_sample_data((uintptr_t) (sp84 + (temp * unk_s6) + sampleAddr),
                                            ALIGN(t0 * unk_s6 + 16, 4), flags,
                                            &synthesisState->sampleDmaIndex, audioBookSample->medium);
                    }

                    a3 = ((uintptr_t) v0_2 & 0xf);
                    aligned = ALIGN(t0 * unk_s6 + 16, 4);
                    addr = (DMEM_ADDR_COMPRESSED_ADPCM_DATA - aligned) & 0xffff;
                    aLoadBuffer(cmd++, VIRTUAL_TO_PHYSICAL2(v0_2 - a3), addr,
                                ALIGN(t0 * unk_s6 + 16, 4));
                } else {
                    s0 = 0;
                    a3 = 0;
                }
                ...
                if (nAdpcmSamplesProcessed == 0) {
                    ...
                    switch (audioBookSample->codec) {
                        case CODEC_ADPCM:
                            aligned = ALIGN(t0 * unk_s6 + 16, 4);
                            addr = (DMEM_ADDR_COMPRESSED_ADPCM_DATA - aligned) & 0xffff;
                            aSetBuffer(cmd++, 0, addr + a3, DMEM_ADDR_UNCOMPRESSED_NOTE + s5Aligned,
                                       s0 * 2);
                            aADPCMdec(
                                cmd++, flags,
                                VIRTUAL_TO_PHYSICAL2(synthesisState->synthesisBuffers->adpcmdecState));
            ...
            switch (nParts) {
                ...
                case 2:
                    switch (curPart) {
                        case 0:
                            aDownsampleHalf(cmd++, ALIGN(samplesLenAdjusted / 2, 3),
                                            DMEM_ADDR_UNCOMPRESSED_NOTE + sp130, DMEM_ADDR_RESAMPLED);
                            resampledTempLen = samplesLenAdjusted;
                            noteSamplesDmemAddrBeforeResampling = DMEM_ADDR_RESAMPLED;
                            if (noteSubEu->finished != FALSE) {
                                aClearBuffer(cmd++,
                                             noteSamplesDmemAddrBeforeResampling + resampledTempLen,
                                             samplesLenAdjusted + 0x10);
                            }
                            break;
                        case 1:
                            aDownsampleHalf(cmd++, ALIGN(samplesLenAdjusted / 2, 3),
                                            DMEM_ADDR_UNCOMPRESSED_NOTE + sp130,
    ...
    if (noteSubEu->usesHeadsetPanEffects) {
        ...
        cmd = note_apply_headset_pan_effects(cmd, noteSubEu, synthesisState, bufLen * 2, flags,
                                             leftRight);
    }
...
u64 *load_wave_samples(u64 *cmd, struct NoteSubEu *noteSubEu, struct NoteSynthesisState *synthesisState,
    ...
    s32 a3;
    s32 repeats;
    aLoadBuffer(cmd++, VIRTUAL_TO_PHYSICAL2(noteSubEu->sound.samples), DMEM_ADDR_UNCOMPRESSED_NOTE,
                128);
...
u64 *final_resample(u64 *cmd, struct NoteSynthesisState *synthesisState, s32 count, u16 pitch,
    ...
    if (pitch == 0) {
        ...
        aResample(cmd++, flags, pitch,
                  VIRTUAL_TO_PHYSICAL2(synthesisState->synthesisBuffers->finalResampleState));
    }
    return cmd;
}

u64 *process_envelope(u64 *cmd, struct NoteSubEu *note, struct NoteSynthesisState *synthesisState,
                      s32 nSamples, u16 inBuf, s32 headsetPanSettings, UNUSED u32 flags) {
    u16 sourceRight;
    u16 sourceLeft;
    u16 targetLeft;
    u16 targetRight;
    ...
    if (note->usesHeadsetPanEffects) {
        ...
        switch (headsetPanSettings) {
            case 1:
                aEnvMixer(cmd++, inBuf, nSamples, (sourceReverbVol & 0x80) >> 7,
                          note->stereoStrongRight, note->stereoStrongLeft, DMEM_ADDR_NOTE_PAN_TEMP,
                          DMEM_ADDR_RIGHT_CH, DMEM_ADDR_WET_LEFT_CH, DMEM_ADDR_WET_RIGHT_CH);
                break;
            case 2:
                aEnvMixer(cmd++, inBuf, nSamples, (sourceReverbVol & 0x80) >> 7,
                          note->stereoStrongRight, note->stereoStrongLeft, DMEM_ADDR_LEFT_CH,
        ...
        aEnvSetup1(cmd++, (sourceReverbVol & 0x7f) * 2, rampReverb, rampLeft, rampRight);
        aEnvSetup2(cmd++, sourceLeft, sourceRight);
        aEnvMixer(cmd++, inBuf, nSamples, (sourceReverbVol & 0x80) >> 7, note->stereoStrongRight,
                  note->stereoStrongLeft, DMEM_ADDR_LEFT_CH, DMEM_ADDR_RIGHT_CH, DMEM_ADDR_WET_LEFT_CH,
